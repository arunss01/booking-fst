...
import { Calendar, MapPin, User, LogOut, PlusCircle, CheckCircle, Search, AlertCircle, Lock, Eye, EyeOff, Hash, Layers, Timer, ShieldAlert, ChevronLeft, ChevronRight } from 'lucide-react';
...
const BookingProvider = ({ children }) => {
  ...
  // NEW: State untuk Tanggal Dashboard
  ...
  const login = async (nimInput, passwordInput) => {
    ...
    if (result.success) {
      setUser(result.user);
      setAuthError("");
      // Tarik data berdasarkan tanggal default (hari ini)
  ...
  // Efek samping: Kalau tanggal dashboard berubah, tarik data baru
  useEffect(() => {
    if (user) {
        refreshUserData(user.id, user.kelas, user.major, dashboardDate);
    }
  }, [dashboardDate]); // <-- Trigger refresh saat tanggal berubah

  const cancelSchedule = async (id, sks) => {
    // Kirim dashboardDate agar server tahu tanggal mana yang dicancel
    ...
    if (result.success) {
      ...
      return { success: true, message: "Jadwal tanggal ini berhasil dibatalkan." };
    }
...
// Login Page (Sama seperti sebelumnya)
const LoginPage = () => {
...
// Dashboard Student (DENGAN FILTER TANGGAL)
const DashboardPage = ({ onChangePage }) => {
  ...
  // Filter jadwal HANYA untuk hari yang dipilih
    ...
    <div className="space-y-8 animate-fade-in pb-24">
      ...
      >
          {modalData.type === 'cancel_schedule' && (
              <p className="text-slate-600">
                  Anda akan membatalkan mata kuliah <strong>{modalData.data?.mataKuliah}</strong> untuk tanggal <strong>{dashboardDate}</strong>.
                  <br/><span className="text-xs text-red-500 font-bold mt-2 block">*Hanya berlaku untuk tanggal ini saja.</span>
              </p>
          )}
          {modalData.type === 'cancel_booking' && <p>Batalkan booking ini?</p>}
      </Modal>
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 flex items-center justify-between sticky top-16 z-30">
        ...
        <input 
            ...
            className="border border-slate-300 rounded-lg px-3 py-2 text-sm font-bold text-slate-700 outline-none focus:ring-2 focus:ring-emerald-500"
        />
      </div>
            ...
            <table className="w-full text-left text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {todaysSchedules.map((sch) => (
                  <tr key={sch.id} className={`hover:bg-slate-50 transition-colors ${sch.status === 'cancelled' ? 'bg-red-50/50' : ''}`}>
                    <td className="p-4 font-medium text-slate-600">{sch.jamMulai} - {sch.jamSelesai}</td>
                    <td className="p-4">
                      <div className={sch.status === 'cancelled' ? 'opacity-50 line-through' : 'font-bold text-slate-800'}>{sch.mataKuliah}</div>
                      <div className="text-xs text-slate-500">{sch.dosen}</div>
                      {sch.status === 'cancelled' && <span className="text-red-600 text-[10px] font-bold uppercase">Dibatalkan (Minggu Ini)</span>}
                    </td>
                    ...
                    <td className="p-4 text-center">
                      {sch.status === 'active' ? (
                        <button onClick={() => handleCancelRequest(sch)} className="bg-white border border-red-200 text-red-600 hover:bg-red-50 px-3 py-1.5 rounded-lg text-xs font-bold shadow-sm">Cancel</button>
                      ) : (
           ...
           <div className="grid gap-4 md:grid-cols-2">
             {myBookings.map(b => (
               <div key={b.bookingId} className={`bg-white rounded-xl shadow-md border p-4 ${b.status === 'DIBATALKAN' ? 'opacity-75 border-red-200' : 'border-slate-200'}`}>
                   ...
                   <p className="text-sm text-slate-500">{b.date} • {b.startTime}</p>
                   {b.status === 'TERKONFIRMASI' && (
                       <button onClick={() => handleBookingCancelRequest(b)} className="mt-3 text-red-500 text-xs font-bold hover:underline">Batalkan Booking</button>
                   )}
               </div>
             ))}
           </div>
...
// Booking Flow (Sama, tidak perlu ubah banyak)
const BookingFlow = ({ onChangePage }) => {
    // Gunakan komponen BookingFlow yang sama persis dengan kode sebelumnya (Hanya copy-paste)
    // ... (Agar jawaban tidak terlalu panjang, pakai BookingFlow dari jawaban sebelumnya)
        ...
        <div className="max-w-lg mx-auto animate-fade-in">
          ...
          <div className="bg-white p-8 rounded-2xl shadow-lg border border-slate-100">
            ...
            <form onSubmit={handleCheckAvailability} className="space-y-6">
              <div><label className="block text-sm font-bold text-slate-700 mb-2">Tanggal</label><input type="date" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl" value={date} onChange={e => setDate(e.target.value)} /></div>
              <div className="grid grid-cols-2 gap-4">
                <div><label className="block text-sm font-bold text-slate-700 mb-2">Jam</label><input type="time" className="w-full p-3 bg-slate-50 border border-slate-200 rounded-xl" value={startTime} onChange={e => setStartTime(e.target.value)} /></div>
    ...
    if (step === 1) {
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {visibleRooms.map((room) => (
                <div key={room.id} className={`p-5 rounded-xl border-2 ${room.isAvailable ? 'bg-white hover:border-emerald-500 cursor-pointer' : 'bg-slate-100 opacity-80'}`} onClick={() => room.isAvailable && requestBooking(room)}>
                  ...
                  {!room.isAvailable && <p className="text-xs text-red-500">{room.conflictReason}</p>}
                </div>
              ))}
            </div>
          </div>
      ...
      return (
        <div className="flex flex-col items-center justify-center h-[60vh] text-center">
            <div className="w-20 h-20 bg-emerald-100 rounded-full flex items-center justify-center mb-4"><CheckCircle className="text-emerald-600" size={40} /></div>
            <h2 className="text-2xl font-bold">Sukses!</h2>
            <p className="font-mono bg-slate-100 p-2 mt-2 rounded">{newBookingId}</p>
        </div>
      )
};
        ...
        <div className="max-w-5xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            <div className="text-right hidden sm:block leading-tight">
              ...
              <div className="text-xs text-emerald-600 font-bold bg-emerald-50 inline-block px-2 py-0.5 rounded-full mt-0.5">{user.major}</div>
            </div>
