...
// PERBAIKAN: Mengimpor fungsi dari file actions.js (asumsi file actions.js berada di direktori yang sama)
...
  
  // HELPER BARU: Menghitung tanggal spesifik dari jadwal
  const getScheduleOccurrenceDate = (dateStr, dayName) => {
      const days = ['MINGGU', 'SENIN', 'SELASA', 'RABU', 'KAMIS', 'JUMAT', 'SABTU'];
      const targetDayIndex = days.indexOf(dayName);
      
      // Membuat objek Date dari dashboardDate (YYYY-MM-DD)
      // Menggunakan 'T00:00:00' untuk menghindari masalah timezone saat perhitungan tanggal
...
      
      // Mengaplikasikan selisih tanggal
      const scheduleDate = new Date(selectedDate);
      scheduleDate.setDate(selectedDate.getDate() + diff);
      
...
  // FUNGSI REVISI: Logika Pembatalan dengan perbandingan Datetime lengkap
  const isSchedulePast = (schedule) => {
      // 1. Tentukan tanggal spesifik jadwal (misal 2025-12-02)
      const scheduleDateStr = getScheduleOccurrenceDate(dashboardDate, schedule.hari);
      
      // 2. Gabungkan tanggal dengan WAKTU SELESAI (jamSelesai)
      // Kita harus menggunakan waktu selesai untuk memastikan pembatalan terkunci hanya setelah kelas usai.
      // Tambahkan 'T' dan ':00' untuk parsing Date yang benar.
      const scheduleEndDatetime = new Date(`${scheduleDateStr}T${schedule.jamSelesai}:00`);
      
      const now = new Date();
      
      // Jadwal dianggap 'Past' jika waktu selesai sudah berlalu atau sama dengan waktu saat ini.
      return scheduleEndDatetime.getTime() <= now.getTime();
  };
  
...
                    const isPast = isSchedulePast(sch); // Menggunakan fungsi revisi
